...
int main()
{
	...
	writedata = 1;
	IOWR_32DIRECT(addr, 24, writedata); // address is base + 4*4 = 16
	// wait for 20ns
	// de-assert the trigger
	writedata = 0;
	IOWR_32DIRECT(addr, 24, writedata);
	...
	for (uint32_t i = 0; i < final_word_address; i += sizeof(uint32_t)) {
		...
		readdata = IORD_32DIRECT(addr, 0); // two pixels

		pix1 = readdata & 0x0000FFFF;
		pix2 = readdata >> 16;
		// each pixel is 16 bits of 5 6 5 RGB format data
		r1 = (pix1 >> 11) * 8; // because we should write 1 byte and red is only 5 bit => 256/32=8
		g1 = ((pix1 & 0b0000011111100000) >> 5) * 4; // 256/64
		b1 = ((pix1 & 0b0000000000011111) * 8); //256/32

		r2 = (pix2 >> 11) * 8; // because we should write 1 byte and red is only 5 bit => 256/32=8
		g2 = ((pix2 & 0b0000011111100000) >> 5) * 4; // 256/64
		b2 = ((pix2 & 0b0000000000011111) * 8); //256/32
